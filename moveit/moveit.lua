...
-- 
-- Used to control things like what to render when and how to process input
-- at different points in the game.  See change_state for a list of values.
g_curstate=0
g_prevstate=0

-- Contains the state for a single level (player position, box positions, etc)
-- See notes.txt for the fields this contains.
g_state={}

-- A list of flags that control what should be rendered in the current frame.
-- The values are:
--  title - the title screen
--  stage - the contents of the stage and all items visible while playing
--  stats - the right portion of the screen, including score, moves and timer
--  level_diag - the start of stage dialog
--  win_diag - the end of stage dialog
--  revert_diag - the retry dialog
--  end_dialog - the 'do you want to quit' dialog
--  main_menu - the main menu
--  logo - the logo screen
--  options - the options submenu
--  credits - the end credits
g_drawflags={title=false,stage=false,stats=false,level_diag=false,win_diag=false,revert_diag=false, 
             end_dialog=false,main_menu=false,logo=false,options=false,credits=false}
...
-- process_input
--
-- Handles key presses on a state-by-state basis
--------------------------------------------------------------------------------
function process_input()
  if g_curstate==0 then           -- logo screen
    if keyp(50)==true then        -- if enter is pressed (a timer will kill this too)
      change_state(1)             -- move to the title screen
    end
  elseif g_curstate==1 then       -- title screen
    if keyp(51) then              -- If backspace is pressed, then exit
      exit()
    elseif any_key()==true then   -- Otherwise, go to main menu state
      change_state(2)
    end    
  elseif g_curstate==2 then       -- main menu
    if keyp(51) then              -- If backspace is pressed, return to title
      change_state(1)
    end
  elseif g_curstate==3 then       -- start level dialog
  elseif g_curstate==4 then       -- in game
    process_movement()            -- defer all processing to the process_movement() function
  elseif g_curstate==5 then       -- win
    if keyp(50) then
      change_state(6)
    end
  elseif g_curstate==7 then       -- revert
    if keyp(25) then              -- yes, revert
      loadlevel(g_state.l)        -- reload the level
      change_state(4)            
    end
    if keyp(14) then              -- no, keep playing
      change_state(4)
    end
  elseif g_curstate==8 then       -- end game
    if keyp(25) then
      change_state(1)
    end
    if keyp(14) then
  ...
  elseif g_curstate==9 then       -- end credits
...
-- check_level_done
-- 
-- Checks to see if the level is done (all the boxes are on targets)
--------------------------------------------------------------------------------
function check_level_done()
...
-- render
-- 
-- the primary render function.  Draws the correct things based on the state.
--------------------------------------------------------------------------------
function render()

  if g_curstate==0 then
    map(30,0)
  elseif g_curstate==1 then
    map(60,0)
  elseif g_curstate==2 then
    map(60,0)
    print('Menu!', 50, 50)
  elseif g_curstate==3 then
  elseif g_curstate==4 then
  elseif g_curstate==5 then
  elseif g_curstate==6 then
  elseif g_curstate==7 then
  elseif g_curstate==8 then
  elseif g_curstate==9 then  
  end
...
-- state_timer_actions
-- 
-- Called every frame to evaluate and act on state-specific timers and counters
--------------------------------------------------------------------------------
function state_timer_actions()
  if g_curstate==0 then
    if time()>=g_title_target then
      change_state(1)
    end
  elseif g_curstate==1 then
  elseif g_curstate==2 then
...
-- change_state
-- 
-- processes a state change - mainly the setting of flags as needed
--
-- State values:
-- 0 = logo
-- 1 = title screen
-- 2 = main menu
-- 3 = start level dialog
-- 4 = in game
-- 5 = win
-- 6 = process next level
-- 7 = revert
-- 8 = end game
-- 9 = end credits
--------------------------------------------------------------------------------
function change_state(target)
  g_prevstate = g_curstate
  g_curstate = target

  -- Do any state-specific initialization here
  if g_curstate==0 then
    -- Set the logo screen to auto transition after 2 seconds
    g_title_target=time()+2000
  elseif g_curstate==1 then
  elseif g_curstate==2 then
  elseif g_curstate==3 then
  elseif g_curstate==4 then
...
-- game_init
--
-- Do all first time setup before we actually let TIC() do stuff freely.
--------------------------------------------------------------------------------
function game_init()
  -- TODO: load data saved into backup storage and set a flag to choose
  -- the default option to highlight on the main menu.  For now, just set
  -- the current level to 1
  g_state.l=1

  -- Initialize to the 'draw logo' state
  g_curstate=0
  g_prevstate=0  
  change_state(0)
...
-- TIC
-- 
-- the primary render function.  Runs 60 times per second.
--------------------------------------------------------------------------------
function TIC()
  if g_init_complete==true then
    state_timer_actions()
    process_input()      
    check_level_done()
    render()  
...
-- 049:2828282802020202282828280202020228282828020202022828282802020202
-- 050:282828280202020228282828020202022828282802020202ffffffffffffffff
-- 051:ffffffffffffffff282828280202020228282828020202022828282802020202
-- </TILES>
...
-- <MAP>
-- 000:
-- 001:
-- 002:
-- 003:
-- 004:
-- 005:
-- 006:
-- 007:
-- 008:
-- 009:
-- 010:
-- 011:
-- 012:
-- 013:
-- 014:
-- 015:
-- 016:
-- </MAP>
